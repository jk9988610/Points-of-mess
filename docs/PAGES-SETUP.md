...
1. 标题栏：**Points-of-mess `v0.1.9`**  
2. 调试首行：`Points-of-mess v0.1.9 已加载…`；测试模式说明含 **「最近 2 轮 messages 历史」**  
3. 第 2 轮起「发AI」的 `messages` 应有多条 `user`/`assistant`，最后一条为结构化 `[game]` 包  
4. **无**「测记忆」按钮（v0.1.8 探测功能已移除）
