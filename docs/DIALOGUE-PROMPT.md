...
模型可能供出 **赵德柱、账房总管** 等未写在 seed 里的专名。程序侧用 **语义闭合** 判定，不绑死「赵爷/老九」：

| 信号 | 含义 |
|------|------|
| `锋利供述` + 指使/主使 + 人名 | 主使轨满足 |
| `唯一主使` / `没有别人` / `没有更高层` | 待证 #1 应删除，禁止再生「是否还有更高层」 |
| 档案含陈四指使链 + 刘老三账本链 | 亮牌消耗可凭档案闭合，不要求玩家复读 offer 原句 |

**禁止**：[已确认] 写「玩家猜测可能掩护他人」；followup 问句经 `sanitizeRhetoricalQuestion` 改陈述。

## 10. v0.5.39（节奏：两轮推进再结局）
