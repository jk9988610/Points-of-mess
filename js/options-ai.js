...
要求：每条 line 为中文一句，≤35 字；followup 必须引用「角色上一句」中的具体词。`;
  ...
  function shouldRegenerateOptions(prev, next) {
    if (optionsUnchanged(prev, next)) {
      return { yes: true, reason: "四轮与上一轮完全相同" };
    }
    const overlap = countOverlappingOptionLines(prev, next);
    if (overlap >= 2) {
      return { yes: true, reason: `有 ${overlap} 条与上一轮相同` };
    }
    return { yes: false };
  }
...
请生成四轮玩家选项 JSON。${avoidBlock}`;
  ...
  async function requestSplitTurn({
    ...
    window.PomDebug?.logLocal("API 路径", "拆分优先 · ①纯文本 reply → ②JSON 四轮选项");
