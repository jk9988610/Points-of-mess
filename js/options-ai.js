...
要求：每条 line 为中文一句，≤35 字；followup 必须引用「角色上一句」中的具体词；只输出 JSON。`;
...
请生成四轮玩家选项 JSON。`;
    ...
    try {
      ...
      parsed = obj.options || obj;
    }
    if (!Array.isArray(parsed)) {
      throw new Error("选项 JSON 不是数组");
    }
    return optionsFromArray(parsed);
  }
  ...
  async function requestCombinedTurn({
    ...
    try {
      ...
      window.PomDebug?.log("合并 JSON 解析失败，尝试兜底", e.message);
      ...
      try {
        ...
        window.PomDebug?.log("选项兜底 API 失败，仍用程序预设", e2.message);
